...
import { createTrade, getAllCoin, getWatchList, removeFromWatchList } from "../../../Redux/coins";
import { removeFromAdminWatchlist } from "../../../Redux/user";
  ...
  const getData = async () => {
    ...
    const res = await dispatch(getAllCoin()).then((res) => {
        let userwatch = watchlist.payload.map((x) => x.coin_name);
             const filterforuserWatchlist = res.payload.filter((x) =>
    ...
    //   console.log("coinData in GetData", filterforuserWatchlist);
    //   localStorage.setItem("cur", JSON.stringify(filterforuserWatchlist));
    //   console.log(res, "res");
    //   console.log(requests, "requests");
    });
  };
...
//         user_id: id,
//         };
//     const res = await dispatch(getWatchList(body)).then((res) => {
//         const filterData = requests?.coinData?.filter((item) => item.name === requests?.userWatchlist?.coin_name);
...
//     };
  useEffect(() => {
    // set 1 min interval for fetching data
      ...
      //const res1= dispatch(getWatchList(body));
      //getAllList();
      //dispatch(getAllCoin());
